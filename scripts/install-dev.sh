#!/bin/bash
# implementation: sogangboard pre-commit and pre-push hook installer
...
install_git_hooks() {
  local magic_str_commit="sogangboard standard pre-commit hook"
  local magic_str_push="sogangboard standard pre-push hook"
